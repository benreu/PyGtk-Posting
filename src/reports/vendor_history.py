	...
	def populate_vendor_purchase_orders (self):
		...
		if self.builder.get_object('checkbutton3').get_active() == True:
			c.execute("SELECT "
						...
						"COALESCE(po.total, 0.00), "
						"COALESCE(po.total, 0.00)::text, "
						"po.paid, "
						"CASE "
							"WHEN ge.check_number IS NOT NULL THEN 'Check #' || ge.check_number "
							"WHEN ge.check_number IS NULL THEN "
								"(SELECT name FROM gl_accounts WHERE number = ge.credit_account) "
							"ELSE '' END, "
						"po.closed, "
						"'Comments:\n' || comments "
						"FROM purchase_orders po "
						"LEFT JOIN gl_entries ge ON ge.gl_transaction_id = "
						"po.gl_transaction_payment_id AND credit_account IS NOT NULL "
