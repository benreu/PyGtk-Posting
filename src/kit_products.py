...
UI_FILE = ui_directory + "/kit_products.ui"
...
class KitProductsGUI:
	...
	def on_drag_data_received(self, widget, drag_context, x,y, data,info, time):
		...
		cursor = DB.cursor()
		cursor.execute("SELECT product, remark, cost FROM %s WHERE id = %s" % (table, _id_))
	...
	def save_assembly_product_line(self, line):
		...
		if line_id == 0:
			cursor.execute("INSERT INTO product_assembly_items (manufactured_product_id, qty, assembly_product_id, remark) VALUES (%s, %s, %s, %s) RETURNING id", (self.manufactured_product_id, qty, product_id, remark))
			line[0] = cursor.fetchone()[0]
		else:
			cursor.execute("UPDATE product_assembly_items SET (qty, assembly_product_id, remark) = (%s, %s, %s) WHERE id = %s", ( qty, product_id, remark, line_id))
		cursor.close()
		DB.commit()
		self.calculate_totals ()
	...
	def delete_entry(self, widget):
		...
		cursor = DB.cursor()
		cursor.execute("DELETE FROM product_assembly_items WHERE id = %s", (line_id,))
		cursor.close()
		DB.commit()

	def add_product(self, widget):
		products.ProductsGUI(manufactured = True)
	...
	def manufactured_row_activate(self, treeview, path, treeviewcolumn):
		...
		cursor = DB.cursor()
		cursor.execute("SELECT pai.id, qty, assembly_product_id, name, "
							"cost, remark "
